                                ...
                                <div class="flex items-center gap-2">
                                    @if($adv->journal_status === 'unposted')
                                        <form method="POST" action="{{ route('driver-advances.post', $adv) }}" class="inline" onsubmit="return confirm('Post driver advance ini ke jurnal?')">
                                            @csrf
                                            <button type="submit" class="tms-btn inline-flex items-center justify-center gap-2 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed font-medium px-3 py-1.5 text-xs btn-primary bg-[var(--color-primary)] hover:bg-[var(--color-secondary)] text-white border border-white/20 shadow-sm hover:shadow-md focus:ring-[var(--color-primary)]" title="Post to Journal">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @else
                                        @if($adv->status === 'pending')
                                            <form method="POST" action="{{ route('driver-advances.unpost', $adv) }}" class="inline" onsubmit="return confirm('Unpost this driver advance from journal?')">
                                                @csrf
                                                <button type="submit" class="tms-btn inline-flex items-center justify-center gap-2 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed font-medium px-3 py-1.5 text-xs hover:bg-slate-100 dark:hover:bg-[#2d2d2d] text-slate-700 dark:text-slate-300" title="Unpost from Journal">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
