    ...
    <x-card>
        <x-slot:header>
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Driver Advances</h1>
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Kelola uang jalan & pelunasan driver</p>
                </div>
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <form method="get" class="flex items-center gap-2">
                        <select name="status" class="rounded-lg bg-white dark:bg-[#252525] border border-slate-300 dark:border-[#3d3d3d] px-4 py-2 text-sm text-slate-900 dark:text-slate-100">
                            ...
                            <option value="pending" @selected(request('status')=='pending')>Pending (Belum DP)</option>
                            <option value="dp_paid" @selected(request('status')=='dp_paid')>DP Paid (Sudah DP)</option>
                            <option value="settled" @selected(request('status')=='settled')>Settled (Lunas)</option>
                        </select>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <x-card :noPadding="true">
            <div class="p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-slate-500 dark:text-slate-400">Pending (Belum DP)</p>
        ...
        <x-card :noPadding="true">
            <div class="p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-slate-500 dark:text-slate-400">DP Paid (Jalan)</p>
        ...
        <x-card :noPadding="true">
            <div class="p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-slate-500 dark:text-slate-400">Settled (Lunas)</p>
                ...
                <tbody class="bg-white dark:bg-[#1e1e1e] divide-y divide-slate-200 dark:divide-[#2d2d2d]">
                    @forelse($advances as $adv)
                        <tr class="hover:bg-slate-50 dark:hover:bg-[#252525] transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <x-button :href="route('driver-advances.show', $adv)" variant="ghost" size="sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm">Belum ada data driver advance</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
