...
<div class="space-y-6">
    ...
    <x-card>
        ...
        <div>
                    <div class="text-2xl font-bold text-slate-900 dark:text-slate-100">Trucks</div>
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Kelola data armada kendaraan</p>
        </div>
        ...
        <form method="get" class="flex items-center gap-2">
                    ...
                    <x-button :href="route('trucks.create')" variant="primary">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody class="bg-white dark:bg-[#1e1e1e] divide-y divide-slate-200 dark:divide-[#2d2d2d]">
                    @forelse($items as $item)
                        <tr class="hover:bg-slate-50 dark:hover:bg-[#252525] transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <x-badge :variant="$item->is_own_fleet ? 'success' : 'warning'">
                                    {{ $item->is_own_fleet ? '🏢 Milik Sendiri' : '🤝 Vendor' }}
                                </x-badge>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <x-button :href="route('trucks.edit', $item)" variant="ghost" size="sm">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </x-button>
                                    <form method="POST" action="{{ route('trucks.destroy', $item) }}" onsubmit="return confirm('Yakin ingin menghapus?')">
                                        @csrf
                                        @method('DELETE')
