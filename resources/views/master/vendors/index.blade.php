...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">Master data vendor</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('vendors.create') }}" class="px-4 py-2 rounded bg-indigo-600 hover:bg-indigo-700 text-white">+ Tambah Baru</a>
            ...
            <tbody>
                @foreach($items as $it)
                    <tr class="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        ...
                        <td class="px-4 py-2">{{ $it->vendor_type }}</td>
                        <td class="px-4 py-2">{{ $it->is_active ? 'Ya' : 'Tidak' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
