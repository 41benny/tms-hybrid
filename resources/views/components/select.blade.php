@props(['label' => null, 'name' => null, 'error' => null, 'required' => false, 'helpText' => null, 'options' => [], 'size' => 'md'])
    ...
    <select 
        {{ $attributes->except('class')->merge([
            'class' => 'w-full rounded-lg bg-white dark:bg-[#252525] border ' . 
                      ...
                      $sizeClasses . ' text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-offset-0 transition-colors shadow-sm [&>option]:bg-white [&>option]:dark:bg-[#252525] [&>option]:text-slate-900 [&>option]:dark:text-slate-100',
            'id' => $name,
            'name' => $name,
        ]) }}
    >
