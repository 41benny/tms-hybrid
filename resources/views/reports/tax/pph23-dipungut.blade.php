@extends('layouts.app', ['title' => 'Laporan PPh 23 Dipungut'])

@section('content')
    <div class="mb-4 flex items-center justify-between">
        <div>
            <div class="text-xl font-semibold">Laporan PPh 23 Dipungut (Dipotong Customer)</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <x-card>
            <div class="text-sm text-slate-600 dark:text-slate-400 mb-1">Total PPh 23 Dipungut (2%)</div>
            <div class="text-2xl font-semibold text-teal-600 dark:text-teal-400">Rp {{ number_format($totalPph23, 0, ',', '.') }}</div>
        </x-card>
    </div>

    <!-- Data Table -->
    <x-card title="Detail PPh 23 yang Dipotong Customer">
        @if($data->count() > 0)
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            ...
                            <th class="px-3 py-3 text-left font-semibold">No. Invoice</th>
                    ...
                    <tbody>
                        @foreach($data as $index => $row)
                            <tr class="border-b border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                ...
                                <td class="px-3 py-2 text-right font-medium text-teal-600 dark:text-teal-400">{{ number_format($row->pph23, 0, ',', '.') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot class="bg-slate-100 dark:bg-slate-800 border-t-2 border-slate-300 dark:border-slate-600">
                        <tr class="font-semibold">
                            ...
                            <td class="px-3 py-3 text-right text-teal-600 dark:text-teal-400">{{ number_format($totalPph23, 0, ',', '.') }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
        @else
            <div class="text-center py-8 text-slate-500 dark:text-slate-400">
                <p>Tidak ada data PPh 23 Dipungut untuk periode ini</p>
            </div>
        @endif
    </x-card>
        ...
        <x-card>
            <div class="text-sm text-slate-600 dark:text-slate-400">
                ...
                <ul class="list-disc list-inside space-y-1">
                    <li>Laporan ini menampilkan PPh 23 yang dipotong customer dari pembayaran invoice perusahaan</li>
                    <li>PPh 23 yang dipotong customer menjadi piutang pajak perusahaan</li>
                    <li>Pastikan bukti potong PPh 23 diterima dari customer untuk keperluan pelaporan</li>
                    <li>Data diambil dari akun 1140 - Piutang PPh 23</li>
                </ul>
            </div>
        </x-card>
    </div>
