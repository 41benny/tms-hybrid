@extends('layouts.app', ['title' => 'Laporan PPN Keluaran'])

@section('content')
    <div class="mb-4 flex items-center justify-between">
        <div>
            <div class="text-xl font-semibold">Laporan PPN Keluaran (Output VAT)</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <x-card>
            <div class="text-sm text-slate-600 dark:text-slate-400 mb-1">Total PPN Keluaran (11%)</div>
            <div class="text-2xl font-semibold text-blue-600 dark:text-blue-400">Rp {{ number_format($totalPpn, 0, ',', '.') }}</div>
        </x-card>
        <x-card>
    ...
    <!-- Data Table -->
    <x-card title="Detail Transaksi PPN Keluaran">
        @if($data->count() > 0)
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            ...
                            <th class="px-3 py-3 text-left font-semibold">No. Faktur</th>
                    ...
                    <tbody>
                        @foreach($data as $index => $row)
                            <tr class="border-b border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                ...
                                <td class="px-3 py-2">
                                    <a href="{{ route('invoices.show', ['invoice' => $row->invoice_number]) }}"
                                ...
                                <td class="px-3 py-2 text-right text-blue-600 dark:text-blue-400">{{ number_format($row->ppn, 0, ',', '.') }}</td>
                    ...
                    <tfoot class="bg-slate-100 dark:bg-slate-800 border-t-2 border-slate-300 dark:border-slate-600">
                        <tr class="font-semibold">
                            ...
                            <td class="px-3 py-3 text-right text-blue-600 dark:text-blue-400">{{ number_format($totalPpn, 0, ',', '.') }}</td>
        ...
        @else
            <div class="text-center py-8 text-slate-500 dark:text-slate-400">
                <p>Tidak ada data PPN Keluaran untuk periode ini</p>
            </div>
        @endif
    </x-card>
        ...
        <x-card>
            <div class="text-sm text-slate-600 dark:text-slate-400">
                ...
                <ul class="list-disc list-inside space-y-1">
                    <li>Laporan ini menampilkan PPN Keluaran dari invoice yang sudah di-post ke jurnal</li>
                    <li>DPP (Dasar Pengenaan Pajak) adalah nilai sebelum PPN</li>
                    <li>PPN Keluaran adalah kewajiban pajak yang harus disetor ke negara</li>
                    <li>Data diambil dari akun 2100 - PPN Keluaran</li>
                </ul>
            </div>
        </x-card>
    </div>
